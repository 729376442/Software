...
namespace EscapeFromTarkovCheat.Utils
{
    public static class Render
    {
        ...
        public static void DrawString(Vector2 position, string label, bool centered = true)
        {
            ...
            var size = StringStyle.CalcSize(content) * 2f;
            var upperLeft = centered ? position - size / 2f : position;
        ...
        public static bool IsPlayerVisible(Player player)
        {
            ...
            // Obtenez le renderer du joueur pour ses bounding box
            Renderer playerRenderer = player.GetComponent<Renderer>();
            if (playerRenderer == null)
                return false;

            // Obtenez les plans du frustum de la caméra
            Plane[] cameraPlanes = GeometryUtility.CalculateFrustumPlanes(Camera.main);

            // Testez si le renderer du joueur est dans le champ de vision
            return GeometryUtility.TestPlanesAABB(cameraPlanes, playerRenderer.bounds);
        }
    }
}